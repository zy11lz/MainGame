module Pro
{
     ...
     * 界面说明： 兑换选择材料
    */
    export class LimitHeroSelectMaterialMediator extends BaseMediator implements IMediator
    {
        ...
        public initUI(): void
        {
            ...
            this.UIPanel.title.text = Global.getLangStr("hero_hupa_ex1",cfg.PetSkinCfgData.getFileNameByPetID(this._allHeros[0].id));
            this.UIPanel.descLbl.text = Global.getLangStr("hero_hupa_ex2");
            this.UIPanel.desc2Lbl.text = Global.getLangStr("hero_hupa_ex9",cfg.PetSkinCfgData.getFileNameByPetID(this._allHeros[0].id));
                        ...
                        else
                        {
                            ...
                            let func = () =>
                            {
                                ...
                                else
                                {
                                    ...
                                    if (tmpInfo.level > 1)
                                    {
                                        TipsUtils.showTipsByLanId("hero_hupa_ex3",cfg.PetSkinCfgData.getFileNameByPetID(tmpInfo.id));
                                        return
                                    }
                                    if (tmpInfo.star > 5)
                                    {
                                        TipsUtils.showTipsByLanId("hero_hupa_ex4",cfg.PetSkinCfgData.getFileNameByPetID(tmpInfo.id));
                                        return;
                                    }
                                    if (tmpInfo.evolve > 0)
                                    {
                                        TipsUtils.showTipsByLanId("hero_hupa_ex5",cfg.PetSkinCfgData.getFileNameByPetID(tmpInfo.id));
                                        return;
                                    }
                                    if (tmpInfo.advance > 0)
                                    {
                                        TipsUtils.showTipsByLanId("hero_hupa_ex6",cfg.PetSkinCfgData.getFileNameByPetID(tmpInfo.id));
                                        return;
                                    }
                                    if (SuitEquipDataMgr.getSuitMgr(tmpInfo.sn).getEquipList().length > 0)
                                    {
                                        TipsUtils.showTipsByLanId("hero_hupa_ex7",cfg.PetSkinCfgData.getFileNameByPetID(tmpInfo.id));
                                        return;
                                    }
                                    if (SuitEquipDataMgr.getSuitMgr(tmpInfo.sn).getGodList().length > 0)
                                    {
                                        TipsUtils.showTipsByLanId("hero_hupa_ex8",cfg.PetSkinCfgData.getFileNameByPetID(tmpInfo.id));
                                        return
                                    }
