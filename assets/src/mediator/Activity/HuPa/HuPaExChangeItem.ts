module Pro
{
    ...
    *
    */
    export class HuPaExChangeItem extends ProUI.Shop.HuPaShopItemViewUI
    {
        ...
        private onSelect(selectHeros: Long[], selectItems: Long[])
        {
            ...
            {
                ActivitySend.drawRewardEx(this._shopCfg.activityID, this._shopCfg.index, 0, selectHeros)
            }
        }
        ...
        private onClickHandler(btn: component.UIButton)
        {
            ...
            {
                //胡帕兑换
                let allList = PetDataMgr.getHuPaList();
                if (allList.length > 0)
                { UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_HuPa_Select_Material, [allList, [], [], [], Math.max(0, this._shopCfg.limitNum - ActivityDataMgr.getHuPaHaveExChangeCount(this._shopCfg.index)), false])); }
                else
                { TipsUtils.showTipsByLanId("activity_title_HuPa_ExChange"); }
            }
            else
            {
        ...
        public setItemByFixShopCfg(shopCfg: cfg.ActivityExchangeExCfgInfo)
        {
            ...
            //所需材料显示
            this.refreshNeedCurrencyInfo(ShopUtils.getNeedItemByExCfgInfo(shopCfg));
        ...
        private refreshNeedCurrencyInfo(addItemInfo: cfg.AddItemInfo)
        {
            if (this._shopCfg.type == 2)
            {
                this.imgCurrency.skin = Global.getHeadPathWithIconName(cfg.PetCfgData.getBaseSkinByPetID(CfgID.PetID.HuPa));
            }
            else
            {
                //道具ID转换成小图标资源
