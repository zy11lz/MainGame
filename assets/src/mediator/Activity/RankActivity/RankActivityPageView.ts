module Pro
{
    ...
    * @author jason.xu
    */
    export class RankActivityPageView extends ProUI.ActivityMain.RankAct.MainViewUI implements ITableView
    {
        /** 对应的活动配置 */
        private _actCfgInfo: cfg.ActivityCfgInfo;
        private _rankActType: Pb_God._emActivityRankType;
        ...
        constructor(actCfgInfo: cfg.ActivityCfgInfo)
        {
            ...
            //this._rankType = Pb_God._emTopListType.TopListType_AcitivtyHero - Pb_God._emActivityRankType.Activity_Rank_Hero + this._rankActType;
            //上面通过精灵榜减去子类型在加上对应子类型获取对应表数据的方法，对表里面的奖励数据顺序有严格的限制，不好用
            //修改方式：活动表榜单类型改为客户端可读，把cs_toplist表对应数据id填进去，下面直接获取榜单类型不在进行计算
            this._rankType = actCfgInfo.topListType;
        ...
        public addEvent(): void
        {
            ...
            EventMgr.on(EventNotify.RankActivity_Title, this, (title) =>
            {
        ...
        {
            CommonHelpView.showWithLangKey(btn, "Help_Rank_Activity_" + this._rankActType);
        }
        ...
        public show(): void
        {
            ...
            let isMerge = cfg.ActivityCfgData.getIsMergeByID(this._actCfgInfo.iD);
            let iTime = isMerge ? TimeController.worldMergeZeroTime : TimeController.worldCreateZeroTime;
            this.bg.skin = isMerge ? "res/rankActivity/xianshichongbang_pic04.jpg" : "res/rankActivity/xianshichongbang_pic03.jpg";
            if (openTimeInfo.isInOpenTime(TimeController.currTimer, iTime))
            {
                //判断是否有礼包可购买的
                this._chargeGiftList = PlatformDataMgr.getValidChargeListByType(Pb_God._emChargeType.ChargeType_ActivityRank, this._rankActType);
                this._diamonBuyList = cfg.ActivityRankCfgData.getListByActivityId(this._actCfgInfo.iD);
                this.btnGift.visible = !isMerge && this._diamonBuyList.length + this._chargeGiftList.length > 0;
                ...
                //活动倒计时
                this._overTargetTime = openTimeInfo.getEndTime(TimeController.currTimer, iTime);
        ...
        private refreshSelfInfoView(selfInfo: Pb_God.PBTopListDetail, beforeOne: Pb_God.PBTopListDetail = null): void
        {
            ...
            let needValueTitle = Global.getLangStr("activity_Rank_RankValue" + this._rankActType);
            ...
            { //排行榜数据里面拿不到。就从活动数据里面去拿。
                selfScoreValue = ActivityDataMgr.getActivity_DataValue(this._actCfgInfo.iD, Pb_God._emActivityDataKey.Activity_Key_RankValue);
            }
            this.txtMyRank.text = rank > 0 ? rank + "" : Global.getLangStr("common_norank");
            let scoreString = needValueTitle + Global.getLangStr("common_fieldbody", selfScoreValue);
        ...
        private refreshRankView(list: Pb_God.PBTopListDetail[]): void
        {
            ...
            this.listRank.onRefresh(len, this, (itemUI: ProUI.ActivityMain.RankAct.RankItemUI, index: number) =>
            {
                ...
                //充值榜特殊性：不显示积分
                itemUI.txtRankValue.visible = this._rankActType != Pb_God._emActivityRankType.Activity_Rank_Charge;
        ...
        private refreshRankRewardView(): void
        {
            ...
            this.listRankReward.onRefreshWithArray(list, this, (itemUI: ProUI.ActivityMain.RankAct.RankRewardItemUI, index: number) =>
            {
                ...
                if (cfgData.limit > 0)
                {
                    ...
                    let scoreTitle = Global.getLangStr("activity_Rank_RankValue" + this._rankActType);
