module Pro
{
     ...
     * 自选礼包
     */
    export class GiftPackSelectMediator extends BaseMediator implements IMediator
    {
        /** UI面板 */
        public UIPanel: ProUI.ItemReview.GiftSelectViewUI;
        ...
        public openUI(): void
        {
            this.showPanel(ProUI.ItemReview.GiftSelectViewUI, 1,BaseAddLayer.CenterUI,true);
        }
        ...
        public initUI(): void
        {
            ...
            this.UIPanel.itemList.onRefresh(this.gift_arr.length, this, this.onListItemRefresh);
        }

        private onListItemRefresh(item: ProUI.ItemReview.GiftSelectItemUI, index: number): void
        {
            let cfg_info = this.gift_arr[index];
            ...
            let itemid = rewards[0].itemid;
            let info: cfg.ItemCfgInfo = cfg.ItemCfgData.getInfo(itemid)
            let petId = -1;
            if(!info.useParam)
            {
                // 该类型不精灵 可能是精灵碎片或是装备
                petId = info.compoundID;
            }
            else
            {
                petId = parseInt(info.useParam.split("_")[0]);
            }
            if(petId < 20000)
            {
                // 装备
            ...
            {
                let godSuitInfo = cfg.GodEquipSuitCfgData.getInfo(petId);
