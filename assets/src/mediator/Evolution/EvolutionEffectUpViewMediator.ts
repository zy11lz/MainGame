...
module Pro
{
	 ...
	 * 进化特效
	 */
	export class EvolutionEffectUpViewMediator extends BaseMediator implements IMediator
	{
		/** UI面板 */
		UIPanel: ProUI.evolution.EvolutionEffectUpViewUI;
		...
		openUI(): void
		{
			this.showPanel(ProUI.evolution.EvolutionEffectUpViewUI, 0, BaseAddLayer.TopUI, true, 3);
		...
		showOnce(): void
		{
			...
			*/
			...
			// Global.setResStarNum(this.UIPanel.StarBox, tmpInfo.star);
		...
		private startShowAni(showStarAni: boolean = false): void
		{
			...
			if (!this._fanhui)
			{
				...
				this._fanhui.on(Laya.Event.START, this, this._onSkPreStart);
				this._fanhui.load(UrlMgr.getSpineSceneUrl("texiao/jinhua/jinhua"));

			}
			this._fanhui.play("effect_up", false);
		}
		...
		private refreshHeroShapeView(useskinid: number, star: number): void
		{
			...
			if (!this._sk)
			{
				...
				//this._sk.pos(0, 100);
				this._sk.pos(0, skinInfo.detalHeight);
