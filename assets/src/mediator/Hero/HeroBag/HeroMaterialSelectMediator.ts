...
{
    /**
     * 界面说明： 英雄材料选择（升星、圣物升阶等用到）
    * @author jason.xu
    */
    export class HeroMaterialSelectMediator extends BaseMediator implements IMediator
    {
        ...
        /** 弹出卸下精灵提示框*/
        private _dischargeBol:boolean=true;
        ...
        public addEvent(): void
        {
            ...
            this.addEventMgr(EventNotify.Pet_State_Chg, this, this.refreshListView);
        }

        /** 模块被【移出舞台】时由 模块管理类 内部自动调用, 无需自己调用, */
        public removeEvent(): void
        {
            this.removeEventMgr(EventNotify.Pet_State_Chg, this, this.refreshListView);
        }
        ...
        private onClickSure(): void
        {
            ...
            EventMgr.trigger(EventNotify.Pet_Materia_Selected, this._selectHeros, this._selectItems, this.UIOpenData.customObject2);
        ...
        private onClickAutoSelect(): void
        {
            ...
            EventMgr.trigger(EventNotify.Pet_Materia_Selected, selectHeros, selectItems, this.UIOpenData.customObject2);
            this.closeUI();
        }
        ...
        private refreshListView(): void
        {
            ...
            this.UIPanel.HeroList.onRefresh(heroCount + itemCount + 1, this, (itemUI: NorItemUI, index: number) =>
            {
                ...
                {  //英雄列表
                    ...
                    // itemUI.onClick(this, () =>
                    // {
                    //     if (PetDataMgr.checkPetOnStore(tmpInfo, true))
                    //     {
                    //         return;
                    //     }
                    //     if (tmpInfo.islock)
                    ...
                    itemUI.onHold(this, (item: NorItemUI, statue: boolean) =>
                    {
                        ...
                        else
                        {
                            ...
                            if(!this._dischargeBol){
                                if(tmpInfo.isDefend){
                                    TipsUtils.showTips(Global.getLangStr("HeroDefendMsg12"));
                                    return;
                                }else if(tmpInfo.isFight){
                                    TipsUtils.showTips(Global.getLangStr("HeroDefendMsg13"));
                                    return;
                                }
                            }
