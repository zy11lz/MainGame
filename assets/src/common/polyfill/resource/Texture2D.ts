 ...
 */
	export class Texture2D extends Laya.BaseTexture
	{
		/**Texture2D资源。*/
		...
		{
			Laya.loader.create(url, complete, null, Laya.Loader.TEXTURE2D);
		}
			...
			switch (this.format)
			{
				...
				case TextureFormat.R32G32B32A32:
					if (LayaGL.layaGPUInstance._isWebGL2)
						gl.texImage2D(textureType, miplevel, (/*<WebGL2RenderingContext>*/gl as any).RGBA32F, width, height, 0, glFormat, gl.FLOAT, pixels);
					else
		...
		loadImageSource(source: any, premultiplyAlpha: boolean = false): void
		{
			...
			{
				(premultiplyAlpha) && (gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, true as any));
				...
				(premultiplyAlpha) && (gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, false  as any));
			}
			if (this._mipmap)
			{
			...
			{//TODO:是否所有图源都可以
				...
				} else
				{
					...
					Laya.Browser.context.drawImage(source, 0, 0, width, height);
					this._pixels = new Uint8Array(Laya.Browser.context.getImageData(0, 0, width, height).data.buffer);//TODO:如果为RGB,会错误
				}
			}
			this._readyed = true;
			this._activeResource();
