...
module Pro
{
	export class IllustrationData_auto extends IllustrationDataMgrBase
	{
		...
		{
			TipsUtils.showTipsByLanId("tips_msg75");

			//这里需要判断一下羁绊的激活
			if (value.indexs)
			{
				for (let i = 0; i < value.indexs.length; i++)
				{
					TipsUtils.showTipsByLanId("tips_msg77", cfg.IllustrationTrammelCfgData.getCombinationtitleById(value.indexs[i]));
				}
			}
		}
		/*****
		...
		{
			TipsUtils.showTipsByLanId("tips_msg76");
			...
			if (value.indexs)
			{
				for (let i = 0; i < value.indexs.length; i++)
				{
					TipsUtils.showTipsByLanId("tips_msg78", cfg.IllustrationTrammelCfgData.getCombinationtitleById(value.indexs[i]));
				}
			}
		}
		/*****
		...
		{
			this._data = value.petdisplay;
			this._historyPower = value.historyPower;
		}
		/***** 请勿添加其他处理函数， 添加将会被覆盖 **/
	}
}