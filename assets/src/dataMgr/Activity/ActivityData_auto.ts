...
module Pro
{
	export class ActivityData_auto extends ActivityDataMgrBase
	{
		...
		{
			//事件转换（参数传递简化，便于红点全局监听检查）
			EventMgr.trigger(EventNotify.Activity_DrawReward, value.id);
		}
		/*****
		 *     活动开始通知，活动ID     PBU32
		 * @param PBU32
		...
		{
			let new_data = new Pb_God.PBPlayerActivityData();
			new_data.id = value.value;
			new_data.resettime = 0;
			new_data.acquired = [];
			new_data.data = [];
			new_data.indexdata = [];
			this._activityDatasMap[value.value] = new_data;
			ActivitySend.getStartTime([new_data.id]);
		}
		/*****
		 *     活动关闭通知，活动ID     PBU32
		 * @param PBU32
		...
		{
			this._activityDatasMap.remove(value.value + "");
			this.activityStartTimeMap.remove(value.value + "");
			EventMgr.trigger(EventNotify.Activity_Close, value.value);
		}
		/*****
		 *     活动数据重置，活动ID     PBU32
		 * @param PBU32
		...
		protected onGetStartTime(value: Pb_God.PBG2CActivityStartTime): void
		{
			...
			{
				this.activityStartTimeMap.put(value.id[i], value.starttime[i]);
				EventMgr.trigger(EventNotify.Activity_OpenTime_Update, value.id[i]);
			}
		}
		/*****
		 *     返回奖励物品的剩余数量   PBG2CActivityRewardNum
		...
		{
			let act_id = value.id;
			this.updateRewardCount(value);
			EventMgr.trigger(EventNotify.Activity_Update, act_id);
		}
		/*****
		 *     同步数据                 PBPlayerActivityData
		...
		{
			this.updateActivityData(value.id, value);

			EventMgr.trigger(EventNotify.Activity_Update, value.id);

			this.checkAndPopupFirstPayUI(value.id);
		}
		/*****
		 *     定制礼包预选商品         PBG2CActivityCustomGiftOrder 
