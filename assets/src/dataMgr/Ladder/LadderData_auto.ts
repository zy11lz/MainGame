...
module Pro
{
	export class LadderData_auto extends LadderDataMgrBase
	{
		...
		{
			this.myRank = value.order;
			if (this.myHistroyTopRank == 0 || this.myHistroyTopRank > this.myRank)  //记录历史最高排名
				this.myHistroyTopRank = this.myRank;
		}
		/*****
		 *刷新对手返回		PBG2CLadderRefreshAck
		...
		{
			this.nextRefreshTime = value.nextrefreshtime;
		}
		/*****
		 *购买次数返回		PBU32
		 * @param PBU32
		...
		{
			let map = value.robot ? this._worshipRobotMap : this._worshipMap;
			if (map.get(value.playerid)) return; //重复了？
			map.put(value.playerid, value.likes);
			this.worshipCount++;
