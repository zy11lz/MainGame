...
module Pro
{
	export class ChampionData_auto extends ChampionDataMgrBase
	{
		constructor()
		{
			super()
			//失败才返回
			EventMgr.on(Cmd.S2C_Champion_Common_Ack.cmdName, this, this.onCommon_Ack)
			//我的竞猜返回		PBG2CChampionSelfGuessAck
			EventMgr.on(Cmd.S2C_Champion_SelfGuessAck.cmdName, this, this.onSelfGuessAck)
			//查询竞猜返回		PBG2CChampionQueryGuessAck
			EventMgr.on(Cmd.S2C_Champion_QueryGuessAck.cmdName, this, this.onQueryGuessAck)
			//竞猜下注同步返回	PBG2CChampionSynGuessAck
			EventMgr.on(Cmd.S2C_Champion_SysGuessAck.cmdName, this, this.onSysGuessAck)
			//查询32强返回		PBG2CChampionQuery32ListAck
			EventMgr.on(Cmd.S2C_Champion_Query32ListAck.cmdName, this, this.onQuery32ListAck)
			//查询4强返回		PBG2CChampionQuery4ListAck
			EventMgr.on(Cmd.S2C_Champion_Query4ListAck.cmdName, this, this.onQuery4ListAck)
			//我的竞猜记录		PBChampionGuessRecord
			EventMgr.on(Cmd.S2C_Champion_GuessRecordAck.cmdName, this, this.onGuessRecordAck)
			//我的战斗记录		PBChampionFightRecord
			EventMgr.on(Cmd.S2C_Champion_FightRecordAck.cmdName, this, this.onFightRecordAck)
			//发送弹幕返回		无内容
			EventMgr.on(Cmd.S2C_Champion_SendDanmuAck.cmdName, this, this.onSendDanmuAck)
			//竞猜结果			PBG2CChampionGuessResultAck
			EventMgr.on(Cmd.S2C_Champion_GuessReusltAck.cmdName, this, this.onGuessReusltAck)
			//查询弹幕返回		PBG2CChampionQueryDanmuAck
			EventMgr.on(Cmd.S2C_Champion_QueryDanmuAck.cmdName, this, this.onQueryDanmuAck)
			//打开返回			PBG2CChampionOpenAck
			EventMgr.on(Cmd.S2C_Champion_OpenAck.cmdName, this, this.onOpenAck)
			//我的结算结果		PBG2CChampionEndResultAck
			EventMgr.on(Cmd.S2C_Champion_EndResultAck.cmdName, this, this.onEndResultAck)
			//查看对战信息		PBChampionBattle
			EventMgr.on(Cmd.S2C_Champion_QueryBattleInfo.cmdName, this, this.onQueryBattleInfo)
			//同步状态			PBG2CChampionSynState
			EventMgr.on(Cmd.S2C_Champion_SynState.cmdName, this, this.onSynState)
			//同步排行结果		PBG2CChampionSynTopResult
			EventMgr.on(Cmd.S2C_Champion_SynTopResult.cmdName, this, this.onSynTopResult)
			//查询对应回合数据	PBG2CChampionQueryRoundAck
			EventMgr.on(Cmd.S2C_Champion_QueryRoundAck.cmdName, this, this.onQueryRoundAck)
			//点赞返回			PBU32U32
			EventMgr.on(Cmd.S2C_Champion_Like.cmdName, this, this.onLike)
		}
		/*****
		 *失败才返回
		 * @param 
		...
		{
			// this.nextOpenTime = value.begintime;
		...
		protected onQueryGuessAck(value: Pb_God.PBG2CChampionQueryGuessAck): void
		{
			...
			this.round = value.round;
			this.state = value.state;
			this.guessCoin = value.guesscoin;
		}
		/*****
		 *竞猜下注同步返回	PBG2CChampionSynGuessAck
		...
		{
			UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_ChampRecord, value));
		}
		/*****
		 *发送弹幕返回		无内容
		 * @param 
		...
		{
			//鹏飞说这个当前排名有值就表示玩家有参与进来(禅道bug:1801)。
			this.isTakePartIn = value.currank > 0;
			this.nextOpenTime = value.begintime;
			this.round = value.round;
			this.state = value.state;
			this.stateOverTime = value.endtime;
		}
		/*****
		 *我的结算结果		PBG2CChampionEndResultAck
		...
		{
			UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_ChampionResult, value));
		}
		/*****
		 *查看对战信息		PBChampionBattle
		...
		protected onSynState(value: Pb_God.PBG2CChampionSynState): void
		{
			...
			this.state = value.state;
			this.stateOverTime = value.endtime;
			//当前如果开始竞猜，并且不在冠军赛界面中，则需要弹窗提示
			if (oldState != this.state && PlayerDataMgr.checkSystemSwitchOpen(emSystemSwitchType.Challenge))
			{
				EventMgr.trigger(EventNotify.Champion_StateChange, this.state);
				let mediator = UIManager.Inst.getUIMeditorInOpenList(PanelNotify.Open_Champion);
				if (this.state == Pb_God._emChampionState._emChampionState_Guess && !(mediator && mediator.getIsPopUp()))
				{
					this.reddotModel.setRedDot(1);
					UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_ChampionPrompt));
				}
			}
		}
		/*****
		...
		{
			if (PlayerDataMgr.checkSystemSwitchOpen(emSystemSwitchType.Challenge))
				UIManager.Inst.pushAutoQueue(new BaseOpenUIData(PanelNotify.Open_ChampionTop3, value));
		}
		/*****
		 *查询对应回合数据	PBG2CChampionQueryRoundAck
