...
module Pro
{
	export class PlayerData_auto extends PlayerDataMgrBase
	{
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_BaseInfo.cmdName);
		}
		/*****
		 * 功能系统信息 PBPlayerSystem
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_SystemInfo.cmdName);
		}
		/*****
		 * 功能系统信息 PBPlayerSystemExt	
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_SystemInfo2.cmdName);
		}
		/*****
		 * 背包信息 PBPlayerBag
		 * @param PBPlayerBag
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_BagInfo.cmdName);
		}
		/*****
		 * 背包信息 PBPlayerBag 扩展
		 * @param PBPlayerBag
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_BagInfo2.cmdName);
		}
		/*****
		 * 伙伴信息 PBPlayerPet
		 * @param PBPlayerPet
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_PetInfo.cmdName);
		}
		/*****
		 * 伙伴信息 PBPlayerPet 扩展
		 * @param PBPlayerPet
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_PetInfo2.cmdName);
		}
		/*****
		 * 伙伴信息3 PBPlayerPetExt	
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_PetInfo3.cmdName);
		}
		/*****
		 * 前端信息 PBClientData
		 * @param PBClientData
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_ClientInfo.cmdName);
		}
		/*****
		 * 好友信息 PBPlayerFriend
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_Friend.cmdName);
		}
		/*****
		 * 邮件信息 PBPlayerMail
		 * @param PBPlayerMail
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_Mail.cmdName);
		}
		/*****
		 * 帮会信息 PBPlayerFaction
		...
		{
			LoginServerMgr.onPlayerInfoLoading(value, Cmd.S2C_Player_Faction.cmdName);
		}
		/*****
		 * 检测角色名字 PBPlayerNameAck
		...
		{
			//这里需要处理在线时长，为了避免时间计算的误差，拿到包以后立即处理，就不等其它队列了。、
			AntiAddictMgr.Inst.init(value.isIDBlind, value.isAdult, value.dailyOnlineTime, value.onlineTime);
		}
		/***** 请勿添加其他处理函数， 添加将会被覆盖 **/
	}
}