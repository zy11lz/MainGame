...
module Pro
{
	export class TopListData_auto extends TopListDataMgrBase
	{
		...
		{
			if (value.info&&value.info.type == Pb_God._emTopListType.TopListType_JoyousLinkup)
			{
				JoyousLinkupDataMgr.myScore = value.info.value.toNumber();
				JoyousLinkupDataMgr.myRank = value.info.rank;
			}
		}
		/*****
		 *返回奖励索引	PBS2CRewardID 
