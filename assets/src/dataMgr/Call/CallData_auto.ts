...
module Pro
{
	export class CallData_auto extends CallDataMgrBase
	{
		...
		{
			let results = this._data.callinfo.filter(elment => elment.calltype == value.calltype);
			if (results.length > 0)
			{
				let callinfo = results[0];
				callinfo.netxfreetime = value.netxfreetime;
				callinfo.totalcount = value.totalcount;
				callinfo.continuecount = value.continuecount;
				callinfo.firstcontinuecount = value.firstcontinuecount;
			}
			else
			{
				this._data.callinfo.push(value);
			}
		...
		protected onChange(value: Pb_God.PBPlayerCallChange): void
		{
			...
			Pro.TipsUtils.showTips(Global.getLangStr("ui_HeroExchange_msg4"));
		}
		/*****
		 * 	 伙伴转换保存返回	PBU32
		 * @param PBU32
		...
		{
			this.isAutoSplitHero = !!value.value;
		}
		/*****
		 * 	 固定伙伴转换返回	PBPlayerCallChange
		 * @param PBPlayerCallChange
		...
		protected onFixChange(value: Pb_God.PBPlayerCallChange): void
		{
			...
			let hero = PetDataMgr.getPetInfo(value.basechangesn);
			UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_HuPa_Liberate_Show, hero));
		}
		/***** 请勿添加其他处理函数， 添加将会被覆盖 **/
	}
}