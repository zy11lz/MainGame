...
module Pro
{
	export class CrossChallengeData_auto extends CrossChallengeDataMgrBase
	{
		...
		protected onRefresh(value: Pb_God.PBCrossChallengeRefresh): void
		{
			if (!this.openInfo)
				return;
			this.openInfo.targets = value.targets;
		}
		/*****
		 *    领取每日宝箱返回	                        PBCrossChallengeDailyInfo
		...
		{
			this.curdailyprize = value.dailyinfo;
			this.challengeReddotModel.refresh(true);
		}
		/*****
		 *	打开跨服竞技场返回						PBCrossChallengeOpenInfo
		...
		{
			this.openInfo = value;
			this.curdailyprize = this.openInfo.dailyinfo;
			this.challengeReddotModel.refresh(true);
			this.honourRedModel.refresh(true);
		}
		/*****
		 *    查询玩家信息返回	                    	PBCrossChallengeInfo
		...
		{
			this.likeplayer.push(value.key);
			this.honourRedModel.refresh(true);

			if (!this.openInfo)
				return;
			for (let i = 0; i < this.openInfo.horourinfo.length; i++)
			{
				if (this.openInfo.horourinfo[i].playerdisplay.playerid == value.key)
				{
					this.openInfo.horourinfo[i].like = value.value;
				}
			}
		...
		protected onSetTeamDEF(value: Pb_God.PBCrossChallengeSetTeamAck): void
		{
			if (!this.openInfo)
				return;
			for (let i = 0; i < value.hideteam.length; i++)
			{
				this.openInfo.self.team[`hideteam${ i + 1 }`] = value.hideteam[i];
			}
