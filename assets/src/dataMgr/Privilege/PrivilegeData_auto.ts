...
module Pro
{
	export class PrivilegeData_auto extends PrivilegeDataMgrBase
	{
		...
		{
			this._buyvippacket.put(value.value, value.value);
			this.reddotModel.getChildModel("vipPage").refreshChild("gift" + value.value)
		}
		/*****
		 *	 购买特权商店 		PBU32
		 * @param PBU32
		...
		{
			let buyCount = this._shopBuyCountMap.get(value.value) || 0;
			this._shopBuyCountMap.put(value.value, buyCount + 1);
		}
		/*****
		 *	 领取每日奖励			PBU32
		 * @param PBU32
		...
		{
			this._dailyPacketMap.put(value.value, true);
			//事件转换（参数传递简化，便于红点全局监听检查）
			EventMgr.trigger(EventNotify.Privilege_Daily_Prize, value.value);
		}
		/*****
		 *	 同步特权卡充值		PBPrivilegeCharge
		...
		{
			this._cardChargeInfo.put(value.cardid, value);
			//事件转换（参数传递简化，便于红点全局监听检查）
			EventMgr.trigger(EventNotify.Privilege_Card_Change, value.cardid);
		}
		/*****
		 *	 同步特权卡			PBPrivilegeCard
		...
		{
			this._privilegeCardMap.put(value.cardid, value.expiretime);
			//事件转换（参数传递简化，便于红点全局监听检查）
			EventMgr.trigger(EventNotify.Privilege_Card_Change, value.cardid);
		}
		/*****
