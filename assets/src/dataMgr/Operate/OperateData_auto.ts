...
module Pro
{
	export class OperateData_auto extends OperateDataMgrBase
	{
		...
		{
			TimeController.ins.onSyncTime(value)
		}
		/*****
		 * 断开命令 	无内容
		 * @param 
		...
		{
			//登陆成功后， 将一些需要立即处理的数据模型先处理了， 其它无需即时的完整数据，待数据收集完整后，在GameLaunch内做统一发送。
			ChatDataMgr.init();
			if (value.curtime) TimeController.ins.setCurServerTime(value.curtime * 1000);
		}
		/*****
		 * 读取超时		无内容
		 * @param 
		...
		{
			EventMgr.trigger(EventNotify.Connect_Server_Close_Prev, value);
		}
		/***** 请勿添加其他处理函数， 添加将会被覆盖 **/
	}
}