...
module Pro
{
	export class TrainData_auto extends TrainDataMgrBase
	{
		...
		{
			let data = this.getTowerData(value.key);
			data.daybuycount = value.value;
		}
		/*****
		 *试练塔领奖返回 			PBU32
		 * @param PBU32
		...
		{
			this._towerPrizeState.put(value.value, value.value);
		}
		/*****
		 *试练塔挑战次数 			PBG2CTowerFightCount
		...
		{
			let data = this.getTowerData(value.type);
			data.dayfightcount = value.dayfightcount;
			data.daybuycount = value.daybuycount;
			data.stageid = value.stageid;
		}
		/*****
		 *无尽试炼领奖				PBU32
		 * @param PBU32
		...
		{
			this._data.endless.prizestage = value.value;
		}
		/*****
		 *无尽试炼选择buff返回		PBU32
		 * @param PBU32
		...
		protected onEndlessBuff(value: Pb_God.PBU32): void
		{
			...
			this._data.endless.buffid = value.value;
		}
		/*****
		 *无尽试炼通知buff组		PBU32
		 * @param PBU32
		...
		{
			this._data.endless.buffgroup = value.value;
		...
		{
			this._data.peak.buff.push(value.value);
			this._peakBuyBuffCount++;
		}
		/*****
		 * 购买次数返回(fight count, buy count) PBU32U32
		 * @param PBU32U32
		...
		{
			this._data.peak.fightcount = value.key;
			this._data.peak.buycount = value.value;
