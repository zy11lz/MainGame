...
module Pro
{
	export class ChallengeData_auto extends ChallengeDataMgrBase
	{
		...
		{
			this.setMyOrder(value.order);
			this.targetList = value.target;
			EventMgr.trigger(EventNotify.Challenge_Target_Update, value.target);
		...
		{
			if (!this.isGetWeekPrize(value.value))
			{
				this._weekPrizeGetList.push(value.value);
				this.reddotModel.refreshChild("challenge");
				EventMgr.trigger(EventNotify.Challenge_WeekPrize_Change, value.value);
			}
		}
		/*****
		 *自己的排行信息	PBG2CChallengeTopInfo
		...
		{
			this.resetEnterCount(value.daycount, value.weekcount);
		}
		/*****
		 *被点赞次数		PBChallengeLikeAck
		...
		{
			this._likePlayerMap.put(value.playerID, value.likeNum);
		}
		/*****
		 *点赞次数		PBU32
		 * @param PBU32
