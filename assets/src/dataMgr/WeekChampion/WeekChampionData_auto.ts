...
module Pro
{
	export class WeekChampionData_auto extends WeekChampionDataMgrBase
	{
		...
		protected onQueryGuessAck(value: Pb_God.PBG2CChampionQueryGuessAck): void
		{
			...
			// this.round = value.round;  // 此处回合为当前竞猜的最后回合数  不是最新回合 用来判断自己是否淘汰
			this.state = value.state;
		...
		{
			UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_WeekChampRecord, value));
		}
		/*****
		 *发送弹幕返回		无内容
		 * @param 
		...
		{
			UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_WeekChampionResult, value));
		}
		/*****
		 *查看对战信息		PBChampionBattle
		...
		protected onSynState(value: Pb_God.PBG2CChampionSynState): void
		{
			...
			{
				EventMgr.trigger(EventNotify.WeekChampion_StateChange, this.state);
				let mediator = UIManager.Inst.getUIMeditorInOpenList(PanelNotify.Open_WeekChampion);
				if (this.state == Pb_God._emChampionState._emChampionState_Guess && !(mediator && mediator.getIsPopUp()))
				{
					this.reddotModel.setRedDot(1);
					UIManager.Inst.forceOpen(new BaseOpenUIData(PanelNotify.Open_WeekChampionPrompt));
				}
			}
		}
		/*****
		...
		{
			UIManager.Inst.pushAutoQueue(new BaseOpenUIData(PanelNotify.Open_WeekChampionTop3, value));
		}
		/*****
		 *查询对应回合数据	PBG2CChampionQueryRoundAck
		...
		{
			this._likePlayerMap.put(value.key, value.value);
		}
		/***** 请勿添加其他处理函数， 添加将会被覆盖 **/
	}
}