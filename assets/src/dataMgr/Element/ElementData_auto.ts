...
module Pro
{
	export class ElementData_auto extends ElementDataMgrBase
	{
		...
		{
			this._data.daybuycount = value.daybuycount;
			this._data.dayfightcount = value.dayfightcount;
		...
		{
			let results = this._data.info.filter(elment => elment.pettype == value.pettype);
			if (results.length == 0)
			{
			...
			{
				results[0].maxstage = value.maxstage;
				results[0].daymaxstage = value.daymaxstage;
			}
		}
		/***** 请勿添加其他处理函数， 添加将会被覆盖 **/
	}
